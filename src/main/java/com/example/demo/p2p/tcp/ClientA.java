			...
			new Thread() {
				public void run() {
					try {
						...
						newsocket.bind(new InetSocketAddress(localIp, localPort));
 
						System.out.println("begin to connect to " + new InetSocketAddress(ip, port)
								+ ", local ip: " + localIp + ", local port: " + localPort);
						...
						while (true) {
							...
							p.write("i am a, hello " + System.currentTimeMillis() + "\n");
							p.flush();
