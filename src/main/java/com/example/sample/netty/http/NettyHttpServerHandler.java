...
public class NettyHttpServerHandler extends SimpleChannelInboundHandler<HttpObject> {
	...
	public void channelRead0(ChannelHandlerContext ctx, HttpObject msg) throws IOException {
		...
		logger.info(">>>>> REQ_URI: {}", uri);
		// 3. 请求头
		HttpHeaders headers = request.headers();
		headers.entries().forEach(entry -> {
			logger.info(">>>>> REQ_HEADER: {} = {}", entry.getKey(), entry.getValue());
		});
		...
		if (HttpMethod.GET.equals(method)) {
			if (uri.endsWith(downloadUri)) { // 下载文件
				...
				if (!file.exists()) {
					logger.info(">>>>> FILE NOT EXISTS: {}", file.getPath());
		...
		// 5. POST 请求
			...
			logger.info(">>>>> ORIGIN_REQ_BODY: {}", reqBody);
			...
			// 5.2 application/x-www-form-urlencoded （表单提交）
				...
				parameters.forEach((key, value) -> {
					logger.info(">>>>> REQ_PARAM: {} = {}", key, value);
					map.put(key, value);
				});
				...
				// 	try {
				// 		logger.info(">>>>> REQ_PARAM: {} = {}", attribute.getName(), attribute.getValue());
			...
			// else if (HttpHeaderValues.MULTIPART_FORM_DATA.toString().equals(contentType)) {
				...
				datas.forEach(data -> {
					...
					// 5.3.2 文件
						...
						logger.info(">>>>> REQ_PARAM: {} = {}", HttpHeaderValues.FILENAME.toString(), filename);
